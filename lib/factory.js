...
function factory( options ) {
	...
	if ( arguments.length ) {
		if ( !isObject( options ) ) {
			throw new TypeError( format( '0p147', options ) );
		}
		...
		if ( hasOwnProp( options, 'state' ) ) {
			...
			if ( opts.state === void 0 ) {
				throw new TypeError( format( '0p17Y', 'state', opts.state ) );
			}
		} else if ( hasOwnProp( options, 'seed' ) ) {
			opts.seed = options.seed;
			if ( opts.seed === void 0 ) {
				throw new TypeError( format( '0p17Y', 'seed', opts.seed ) );
			}
		}
		if ( hasOwnProp( options, 'copy' ) ) {
			opts.copy = options.copy;
			if ( !isBoolean( opts.copy ) ) {
				throw new TypeError( format( '0p12o', 'copy', opts.copy ) );
			}
		}
	}
	prng = PRNGS[ opts.name ];
	if ( prng === void 0 ) {
		throw new Error( format( '0p17Z', opts.name ) );
	}
	if ( opts.state === void 0 ) {
		if ( opts.seed === void 0 ) {
			rand = prng.factory();
