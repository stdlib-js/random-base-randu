...
function factory( options ) {
	...
	if ( arguments.length ) {
		if ( !isObject( options ) ) {
			throw new TypeError( format( '0PP4K', options ) );
		}
		...
		if ( hasOwnProp( options, 'state' ) ) {
			...
			if ( opts.state === void 0 ) {
				throw new TypeError( format( '0PP7q', 'state', opts.state ) );
			}
		} else if ( hasOwnProp( options, 'seed' ) ) {
			opts.seed = options.seed;
			if ( opts.seed === void 0 ) {
				throw new TypeError( format( '0PP7q', 'seed', opts.seed ) );
			}
		}
		if ( hasOwnProp( options, 'copy' ) ) {
			opts.copy = options.copy;
			if ( !isBoolean( opts.copy ) ) {
				throw new TypeError( format( '0PP30', 'copy', opts.copy ) );
			}
		}
	}
	prng = PRNGS[ opts.name ];
	if ( prng === void 0 ) {
		throw new Error( format( '0PP7r', opts.name ) );
	}
	if ( opts.state === void 0 ) {
		if ( opts.seed === void 0 ) {
			rand = prng.factory();
